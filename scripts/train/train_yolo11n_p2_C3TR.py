...
from scripts.common.paths import project_path, resolve_pretrained
...
def main():
    ...
    model = YOLO(str(project_path("configs", "models", "yolo11n_p2_C3TR.yaml"))).load(
        resolve_pretrained("yolo11n.pt")
    )
    ...
    # 2. 启动训练
    results = model.train(
        data=str(project_path("configs", "datasets", "visdrone_custom.yaml")),
        epochs=150,
